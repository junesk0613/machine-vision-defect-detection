...
- **AI 불량 검출**: YOLOv8s(ONNX Runtime)로 PCB 앞면 부품 누락 등 14개 클래스 판정
...
│   ├── App.xaml.cs           #   앱 진입점
│   ├── AppConfig.cs          #   Flask URL 등 앱 환경 설정
│   ├── Models.cs             #   검사 결과 데이터 모델
│   ├── LoginWindow.xaml.cs   #   로그인 화면
│   ├── MainWindow.xaml.cs    #   메인 UI / 공정 제어 로직
│   ├── ApiService.cs         #   Flask 백엔드 HTTP / Socket.IO 통신
│   ├── DbService.cs          #   사용자 인증 (DB 로그인)
│   ├── CameraService.cs      #   카메라 영상 캡처 · PCB 윤곽 검출
│   ├── YoloService.cs        #   YOLO(ONNX) 앞면 불량 추론
│   ├── OcrService.cs         #   시리얼 번호 OCR
│   ├── ReportService.cs      #   PDF / Excel 리포트 생성
│   └── SerialService.cs      #   RS-232 아두이노 시리얼 통신
│
